...
abstract class Level extends Screen {
  ...
  void showText(boolean b)        { for(LevelLayer l: layers) { l.showText = b; }}
...
abstract class LevelLayer {
  ...
  // same for text
  void addText(Drawable fixed)    { fixed_text.add(fixed);    }
  void removeText(Drawable fixed) { fixed_text.remove(fixed); }
  void clearText() { fixed_text.clear(); }
  ...
  void draw() {
    ...
    if (showText)   	  handleText(x,y,w,h);
   ...
   * Draw all Texts
   */
  void handleText(float x, float y, float w, float h) {
    for(LevelText s: fixed_text) {
      s.draw(x,y,w,h);
    }
  }
...
class ViewBox {
  ...
  void track(Level level, Actor who) {
	...
	// println("VIEWPORT: track hero. set to " + x + "/" + y);
