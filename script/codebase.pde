...
// ====== KEY HANDLING ======
  ...
  protected boolean isKeyDown(char key) {
    int keyCode = int(key);
	println( "isKeyDown called with " + key + " will return " + keyDown[keyCode]);
    return keyDown[keyCode];
  }
  
  protected boolean noKeysDown() {
    for(i=0; i < keyDown.size(); i++) { boolean b = keyDown.get(i); if(b) return false; }
    for(i=0; i < locked.size(); i++) { boolean b = locked.get(i); if(b) return false; }
    return true;
  }
...
class Boundary extends Positionable {
  ...
  void notifyListeners(Actor actor, float[] correction) {
    for(i=0; i < listeners.size(); i++) {
	  BoundaryCollisionListener l = listeners.get(i); 
...
// draw loop
void draw() { 
  println( "draw loop called");
  activeScreen.draw(); 
  // SoundManager.draw();
  println( "draw loop END");
}
...
abstract class Level extends Screen {
  ...
  void showBackground(boolean b)  { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showBackground = b; }}
  void showBoundaries(boolean b)  { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showBoundaries = b; }}
  void showPickups(boolean b)     { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showPickups = b; }}
  void showDecals(boolean b)      { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showDecals = b; }}
  void showInteractors(boolean b) { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showInteractors = b; }}
  void showActors(boolean b)      { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showActors = b; }}
  void showForeground(boolean b)  { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showForeground = b; }}
  void showTriggers(boolean b)    { for(i=0; i < layers.size(); i++) { LevelLayer l = layers.get(i); l.showTriggers = b; }}
...
abstract class LevelLayer {
  ...
  void handleBackground(float x, float y, float w, float h) { 
    ...
    for(i=0; i < fixed_background.size(); i++) {
	  Drawable s = fixed_background.get(i);
      s.draw(x,y,w,h);
    }
  }
  ...
  void handleBoundaries(float x, float y, float w, float h) { 
    ...
    for(i=0; i < bounded_interactors.size(); i++) {
	  BoundedInteractor b = bounded_interactors.get(i);
      if(b.bounding) {
    ...
    for(int i = pickups.size()-1; i>=0; i--) {
      ...
      if(p.interacting && p.inMotion && !p.onlyplayerinteraction) {
        for(j=0; j < boundaries.size(); j++) {
		  Boundary b = boundaries.get(j);
          CollisionDetection.interact(b,p); }
        for(j=0; j < bounded_interactors.size(); j++) {
		  BoundedInteractor o = bounded_interactors.get(j);
          if(o.bounding) {
            for(k=0; k < boundaries.size(); k++) {
				Boundary o = boundaries.get(k);
                CollisionDetection.interact(b,p); }}}}

      // player interaction?
      for(int j =0; j < players.size(); j++) {
	    Player a = players.get(j);
    ...
    for(int i = npcpickups.size()-1; i>=0; i--) {
      ...
      if(p.interacting && p.inMotion && !p.onlyplayerinteraction) {
        ...
        for(j=0; j < bounded_interactors.size(); j++) {
		  BoundedInteractor o = bounded_interactors.get(j);
          if(o.bounding) {
            for(k=0; k < o.boundaries.size(); k++) {
				Boundary b = o.boundaries.get(k);
                CollisionDetection.interact(b,p); }}}}

      // npc interaction?
      for(j=0; j < interactors.size(); j++) {
	    Interactor a = interactors.get(j);
        if(!a.interacting) continue;
        float[] overlap = a.overlap(p);
        if(overlap!=null) {
          p.overlapOccurredWith(a);
    ...
    for(int i=players.size()-1; i>=0; i--) {
      ...
      if(a.interacting) {
        ...
        if(a.inMotion) {
          ...
          // boundary interference from bounded interactors?
          for(int j=0; j < bounded_interactors.size(); j++) {
		    BoundedInteractor o = bounded_interactors.get(j);
            if(o.bounding) {
              for(int k=0; i < o.boundaries.size(); k++) {
			    Boundary b = o.boundaries.get(k);
                CollisionDetection.interact(b,a); }}}}
  ...
  void handleForeground(float x, float y, float w, float h) {
    for(int i=0; i < fixed_foreground.size(); i++) {
	  Drawable s = fixed_foreground.get(i);
      s.draw(x,y,w,h);
    }
  }
